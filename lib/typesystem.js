...
function TypeSystem() {
    ...
    function checkType(value, name, type) {
        if (!testerMap[type](value)) {
            throw new TypeError(
                'The <' + name + '> must be of type: ' + quote(type)
            );
        }
    ...
    function defineType(name, supertype, tester) {
        ...
        if (!/^[a-z]+$/.test(name)) {
            throw new SyntaxError(
                'The <name> must consist only of lowercase letters.'
            );
        }
    ...
    self.defineInt = function () {
        return defineType('int', 'primitive:number', function (number) {
