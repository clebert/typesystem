...
function createOutOfBoundsError(adverb, actual, expected) {
    return new Error('Expected ' + adverb + ' ' + expected + ' arguments, but got ' + actual + ' arguments.');
}
...
function TypeSystem() {
    ...
    function checkBounds(args, min, max) {
        ...
        if (min === max) {
            if (length !== min) {
                throw createOutOfBoundsError('exactly', length, min);
            }
        } else {
            if (length < min) {
                throw createOutOfBoundsError('at least', length, min);
            }

            if (length > max) {
                throw createOutOfBoundsError('at most', length, max);
            }
        }
